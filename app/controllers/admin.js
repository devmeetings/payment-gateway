...
function renderDiploma (req, res, next) {
    ...
    Claims.find(condidtions).populate('event').exec(intercept(next, function (claims) {

        if (!claims || claims.length === 0) {
            res.status(500).send('No claims');
            return;
        }

        Country.findOne({
            _id: claims[0].event.country
            ...
            req.i18n.changeLanguage(lng, function () {
                var eventStartDate = moment(claims[0].event.eventStartDate).format('LL');
                res.render('diploma/diploma', {
                    eventStartDate: eventStartDate,
                    users: JSON.stringify(claims)
                });
...
router.get('/events/:ev/claims', function (req, res, next) {
    ...
    Claims.find({
        ...
        Q.all([
            tickets.countClaimsByStatus(req.params.ev, Claims.STATUS.PAYED),
            tickets.countClaimsByStatus(req.params.ev, Claims.STATUS.PENDING),
            tickets.countClaimsByStatus(req.params.ev, Claims.STATUS.OFFLINE_PENDING),
            tickets.countClaimsForSponsor(req.params.ev),
            tickets.countClaimsForVip(req.params.ev)
...
router.post('/events/:ev/cancel/:claimId', tickets.cancelTicket);

router.post('/events/:ev/offline/:claimId', tickets.setTicketAsPendingForOfflinePayment);

router.post('/events/:ev/add/claim/offline', tickets.addNewClaimFromAdminPanel);

router.post('/events/:ev/invitation', function (req, res, next) {

    function createClaim (ev) {
        var validTill = new Date(claimTimeToPay());
        Claims.create({
            event: ev._id,
